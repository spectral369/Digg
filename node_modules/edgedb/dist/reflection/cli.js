...
`);
    ...
    while (currentDir !== systemRoot) {
        if (await (0, adapter_node_1.exists)(adapter_node_1.path.join(currentDir, "edgedb.toml"))) {
...
Run this command inside an EdgeDB project directory or specify the desired target language with \`--target\``);
        ...
        const denoConfigPath = adapter_node_1.path.join(projectRoot, "deno.json");
        const denoJsonExists = await (0, adapter_node_1.exists)(denoConfigPath);
        let packageJson = null;
        const pkgJsonPath = adapter_node_1.path.join(projectRoot, "package.json");
        if (await (0, adapter_node_1.exists)(pkgJsonPath)) {
            packageJson = JSON.parse(await (0, adapter_node_1.readFileUtf8)(pkgJsonPath));
        }
        if (denoJsonExists) {
            options.target = "deno";
            console.log(`Detected deno.json, generating TypeScript files with Deno-style imports.`);
        }
   ...
   Run \`npx edgeql-js --help\` for full options.`;
    ...
    if (options.outputDir) {
        outputDir = adapter_node_1.path.isAbsolute(options.outputDir)
            ? options.outputDir
            : adapter_node_1.path.join(process.cwd(), options.outputDir);
    }
    else if (projectRoot) {
        outputDir = adapter_node_1.path.join(projectRoot, "dbschema", "edgeql-js");
    }
    else {
